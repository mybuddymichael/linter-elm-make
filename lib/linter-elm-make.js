...
module.exports = {
  config: {
    ...
    alwaysCompileMain: {
      ...
      description:  'Always compile `Main.elm` instead of the active file. This will only work if `Only Lint On Save` is checked, for now.',
      type: 'boolean',
      default: false,
      order: 3
    },
  ...
  activate() {
    ...
    let clearElmEditorFixes = (editor) => {
      const editorPath = editor.getPath();
      if (this.quickFixes[editorPath]) { delete this.quickFixes[editorPath]; }
    };
    let subscribeToElmEditorEvents = (editor) => {
      editor.onDidChange(() => { clearElmEditorFixes(editor); });
      editor.onDidDestroy(() => { clearElmEditorFixes(editor); });
      // TODO When do we delete the editor's entry in `this.problems`?
    };
  ...
  quickFixAll() {
    ...
    if (quickFixes) {
      ...
      // Clear editor's quick fixes since buffer already changed.
      delete this.quickFixes[editor.getPath()];
    }
  },
  provideLinter() {
    const proc = process;
          ...
          if (atom.config.get('linter-elm-make.onlyLintOnSave')) {
            if (atom.config.get('linter-elm-make.alwaysCompileMain')) {
              let sourceDirectories = (() => {
                try {
                  return JSON.parse(fs.readFileSync(path.join(projectRootDirectory, 'elm-package.json')).toString())['source-directories'];
                } catch (error) {
                }
              })();
    ...
    this.subscriptions.add(atom.config.observe('linter-elm-make.onlyLintOnSave', onlyLintOnSave => {
        ...
        if (!onlyLintOnSave && atom.config.get('linter-elm-make.alwaysCompileMain')) {
          // Uncheck `Always Compile Main` automatically.
  ...
  doLint(editorFilePath, inputFilePath, projectRootDirectory, editor) {
    ...
    .then(data => {
      return new Promise((resolve, reject) => {
        if (data.stderr === '') {
          self.parseStdout(data.stdout, editorFilePath, inputFilePath, projectRootDirectory, editor, resolve);
        } else {
          self.parseStderr(data.stderr, editorFilePath, resolve);
        }
  ...
  parseStdout(stdout, editorFilePath, inputFilePath, projectRootDirectory, editor, resolve) {
    ...
    for (let filePath of uniqueFilePaths) {
      this.problems[filePath] = allProblems.filter(({filePath}) => filePath);
    }
  ...
  computeFixesForEditor(editor) {
    const filePath = editor.getPath();
    if (this.problems[filePath]) {
      const quickFixes =
        this.problems[filePath]
        .map(({problem, range}) => {
          return {
