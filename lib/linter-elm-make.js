...
import SymbolsMatchingTypeView from './symbols-matching-type-view';
...
export default {
  ...
  activate() {
    ...
    this.symbolsMatchingTypeView = new SymbolsMatchingTypeView();
    ...
    this.quickFixView.onDidConfirm(({ editor, range, fix }) => {
      quickFixing.fixProblem(
        ...
        range,
        fix,
        self.getSymbolsMatchingType,
        self.showSymbolsMatchingType.bind(self)
      );
      self.clearElmEditorProblemsAndFixes(editor);
    });
    this.symbolsMatchingTypeView.onDidConfirm(({ editor, range, symbol }) => {
      const fix = {
        type: 'Replace with',
        text:
          '(' + editor.getTextInBufferRange(range) + ' |> ' + symbol.name + ')',
      };
      quickFixing.fixProblem(editor, range, fix, null, null);
  ...
  deactivate() {
    ...
    this.symbolsMatchingTypeView.destroy();
    this.symbolsMatchingTypeView = null;
    this.prevElmEditor = null;
  ...
  showSymbolsMatchingType(editor, range, symbols) {
    this.symbolsMatchingTypeView.show(editor, range, symbols);
  },
  ...
  clearProjectBuildArtifactsCommand() {
    ...
    const self = this;
    getProjectBuildArtifactsDirectory(
      editorFilePath
  ...
  consumeGetSymbolsMatchingType(getSymbolsMatchingType) {
    this.getSymbolsMatchingType = getSymbolsMatchingType;
  },
  ...
  executeElmMake(editorFilePath, inputFilePath, cwd, editor, projectDirectory) {
    ...
    const timeout = isNaN(timeoutConfig)
      ? 10000
      : timeoutConfig === 0 ? Infinity : timeoutConfig * 1000;
  ...
  maybeShowInferredTypeAnnotations(editor) {
    ...
    if (atom.config.get('linter-elm-make.showInferredTypeAnnotations')) {
      this.quickFixesWhere(
        ...
        this.subscriptions.add(
          marker.onDidDestroy(() => {
            let markerIndex = this.typeAnnotationMarkers[editor.id].indexOf(
              marker
            );
            if (markerIndex != -1) {
