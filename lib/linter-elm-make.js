...
module.exports = {
        ...
        if (atom.config.get('linter-elm-make.lintOnTheFly')) {
          ...
          if (projectDirectory) {
            ...
            if (workDirectory) {
              ...
              deleteBuildArtifacts(workDirectoryBuildArtifactsDirectory);
              if (atom.inDevMode()) {
  ...
  watchProjectDirectory(projectDirectory, workDirectory) {
    ...
    let ignored = ['**' + path.sep + 'elm-stuff' + path.sep + '**' + path.sep + '*.elm'];
    if ((workDirectory + path.sep).startsWith(projectDirectory)) {
    ...
    watcher.on('add', (filename) => {
      ...
      if (!filePath.startsWith(workDirectory + path.sep) && (path.basename(filePath) === 'elm-package.json' || path.extname(filePath) === '.elm')) {
        if (atom.inDevMode()) {
    ...
    watcher.on('change', (filename) => {
      const filePath = path.join(projectDirectory, filename);
      if (!filePath.startsWith(workDirectory + path.sep) && path.basename === 'elm-package.json') {
        forceLintActiveElmEditor();
      }
    });
