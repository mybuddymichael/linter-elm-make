...
function formatExpectedType(expected, actual) {
  const diff = jsdiff.diffWords(actual, expected);
  return '<span class="linter-elm-make-details-type">' +
    diff.map((part) => {
      if (part.added) {
...
function formatExpectedName(expected, actual) {
  ...
  return '<span class="linter-elm-make-details-name">' +
    diff.map((part) => {
      if (part.added) {
...
function formatHint(hint) {
  ...
  regex = /^(I am seeing issues with the )(.+)( fields.((?:.|\n)+))$/;
  matches = hint.match(regex);
  if (matches && matches.length > 3) {
    ...
    if (nameParts.length === 1) {
      ...
      formattedNames = nameParts.map((name) => { return formatInfo(name); }).join(', ') + ' and ' + formatInfo(last.replace('and ', ''));
    }
...
function formatProblemDetails(problem) {
  ...
  switch (problem.tag) {
    ...
    case 'NAMING ERROR':
      ...
      matches = problem.details.match(regex);
      if (matches && matches.length > 1) {
        const suggestions = matches[1];
        return problem.details.replace(regex,
          'Maybe you want one of the following?\n\n' +
          formatInfo(suggestions));
      }
      return problem.details;
    case 'PORT ERROR':
