...
export default {
    ...
    this.subscriptions.add(atom.workspace.observeActivePaneItem((item) => {
      if (item && isElmEditor(item)) {
        if (!atom.config.get('linter-elm-make.showInferredTypeAnnotations')) {
          destroyMarkers(self.typeAnnotationMarkers, item.id);
        }
        if (atom.config.get('linter-elm-make.lintOnTheFly')) {
  ...
  deactivate() {
    ...
    Object.keys(this.typeAnnotationMarkers).forEach((editorId) => {
      destroyMarkers(this.typeAnnotationMarkers, editorId);
    });
    this.typeAnnotationMarkers = null;
    Object.keys(this.extraHintMarkers).forEach((editorId) => {
      destroyMarkers(this.extraHintMarkers, editorId);
    });
    this.extraHintMarkers = null;
    Object.keys(this.subregionMarkers).forEach((editorId) => {
      destroyMarkers(this.subregionMarkers, editorId);
    });
    this.subregionMarkers = null;
  },
  ...
  provideLinter() {
    ...
    this.subscriptions.add(atom.config.observe('linter-elm-make.showInferredTypeAnnotations', show => {
      if (!show) {
        const editor = atom.workspace.getActiveTextEditor();
        if (isElmEditor(editor)) {
          destroyMarkers(self.typeAnnotationMarkers, editor.id);
        }
      }
    }));
    return linter;
    ...
    const problemsByLine = stdout.split('\n').map((line) => {
      ...
      if (!json) {
        ...
        return json.map((problem) => {
          ...
          const range = regionToRange(problem.region);
          const subrange = regionToRange(problem.subregion);
          ...
          return {
            type: problem.type,
            html: `<a class='${getAnchorForMessage(filePath, range, subrange)}'></a>${colorize(_.escape(problem.overview))}<br/><br/>${colorize(_.escape(problem.details).split('\n').join('<br/>&nbsp;'))}`,
            filePath: filePath,
            range: range,
            subrange: subrange,
            problem: problem
  ...
  maybeShowInferredTypeAnnotations(editor) {
    // Remove old type annotation markers.
    destroyMarkers(this.typeAnnotationMarkers, editor.id);
    ...
    if (atom.config.get('linter-elm-make.showInferredTypeAnnotations')) {
      this.quickFixesWhere(editor, fix => fix.type === 'Add type annotation')
        .forEach(({range, fixes}) => {
          ...
          let decor = editor.decorateMarker(marker, {type: 'block', position: 'before', item: element});
          this.typeAnnotationMarkers[editor.id].push(marker);
        });
    }
  },
  markSubregions(editor) {
    // Remove old subregion markers.
    destroyMarkers(this.subregionMarkers, editor.id);
    this.subregionMarkers[editor.id] = [];
    const problems = this.problems[editor.getPath()];
    this.problems[editor.getPath()]
      .forEach(({subrange, type}) => {
        if (subrange) {
          let marker = editor.markBufferRange(subrange, {invalidate: 'touch'});
          editor.decorateMarker(marker, {type: 'highlight', class: 'linter-elm-make-subregion-' + type});
          this.subregionMarkers[editor.id].push(marker);
        }
      });
  },
  maybeShowExtraHints(editor) {
    // Remove old hint markers.
    destroyMarkers(this.extraHintMarkers, editor.id);
  ...
  maybeScrollProblemAtCursorIntoView() {
    if (atom.config.get('linter-elm-make.autoscrollIssueIntoView')) {
      ...
      if (linterPanel) {
        ...
        if (problem) {
          let problemAnchor = linterPanel.getElementsByClassName(getAnchorForMessage(problem.filePath, problem.range, problem.subrange));
  ...
  getProblemAtCursorPosition() {
    ...
    if (problems) {
      ...
      if (problemsAtPosition.length > 0) {
        // Get the most specific range or subrange.
        return _.min(problemsAtPosition,
          ({range, subrange}) => {
            const testRange = subrange || range;
            return testRange.end.row - testRange.start.row;
          });
      }
    }
    return null;
...
function getAnchorForMessage(filePath, range, subrange) {
  return 'linter-elm-make://' + filePath + ':' + range.start.row + ',' + range.start.column +
    (subrange ? (':' + subrange.start.row + ',' + subrange.start.column) : '');
}
...
function destroyMarkers(editorMarkers, editorId) {
