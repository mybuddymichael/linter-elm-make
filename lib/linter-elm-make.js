...
module.exports = {
  config: {
    ...
    alwaysCompileMain: {
      ...
      description: 'Always compile `Main.elm` instead of the active file.  Modules unreachable from the main modules will not be linted.',
      type: 'boolean',
      default: true,
      order: 3
    },
    ...
    atom.workspace.observeActivePaneItem((item) => {
      if (atom.config.get('linter-elm-make.lintOnTheFly') &&
        ...
        if (self.prevElmTextEditor) {
          // When a text editor loses focus, update the associated temporary file.
        ...
        if (editorFilePath) {
          ...
          if (atom.config.get('linter-elm-make.lintOnTheFly')) {
            ...
            if (!self.tempLintDirectories[projectDirectory]) {
              ...
              // TODO When to close watcher, delete temporary directory, and delete self.tempLintDirectories[projectDirectory]?
  ...
  getMainFilePath(projectDirectory) {
    ...
    if (sourceDirectories) {
      // TODO Get the `Main.elm` files in all source directories.
  ...
  compileInTempDirectory(editorFilePath, tempLintDirectory, editor, projectDirectory) {
    ...
    if (atom.config.get('linter-elm-make.alwaysCompileMain')) {
      ...
      if (mainFilePath) {
        // TODO Compile the `Main.elm` file of each source directory, then combine the linting results.
