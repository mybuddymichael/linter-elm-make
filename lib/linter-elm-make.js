...
module.exports = {
  ...
  activate() {
    ...
    atom.commands.add('atom-text-editor', {
      ...
      'linter-elm-make:compute-dependency-graph': module.exports.computeDependencyGraph
    });
    atom.workspace.observeTextEditors((textEditor) => {
      textEditor.onDidDestroy(() => {
        if (module.exports.quickFixes[textEditor.getPath()]) {
          delete module.exports.quickFixes[textEditor.getPath()];
        }
      });
    });
  ...
  computeDependencyGraph() {
    ...
    const graph = getDependencyGraph(path.join(projectRootDirectory, sourceDirectory), 'Main');
  ...
  quickFix() {
    ...
    const quickFixes = module.exports.quickFixes[textEditor.getPath()];
    if (quickFixes) {
        ...
        if (filePath) {
          if (!atom.config.get('linter-elm-make.onlyLintOnSave')) {
            return helpers.tempFile(path.basename(filePath), textEditor.getText(), (tempFilePath) => {
              return doLint(filePath, tempFilePath, textEditor);
            });
          } else {
            return doLint(filePath, filePath, textEditor);
          }
        } else {
          return [];
        }
...
function getDependencyGraph(directory, module) {
  const filePath = path.join(directory, module + '.elm');
  if (fs.existsSync(filePath)) {
    const importedModules = getImportedModules(filePath);
    console.log(importedModules); // // //
    if (importedModules.length === 0) {
      return [filePath];
    }
    return importedModules.map(importedModule => {
      getDependencyGraph(importedModule.replace(/\./g, '/') + '.elm');
    });
  }
}

function getImportedModules(filePath) {
  console.log('getImportedModules');
  return fs.readFileSync(filePath).toString()
    .match(/^import (.+)(\s|$)/gm)
    .map((line) => {
      return line.match(/^import (.+?)(\s|$)/)[1];
    });
}

function doLint(filePath, inputFilePath, textEditor) {
  ...
  .then(data => {
    return new Promise((resolve, reject) => {
      if (data.stderr === '') {
        parseStdout(data.stdout, filePath, inputFilePath, projectRootDirectory, textEditor, resolve);
      } else {
        parseStderr(data.stderr, filePath, resolve);
      }
...
function parseStdout(stdout, filePath, inputFilePath, projectRootDirectory, textEditor, resolve) {
...
function parseStderr(stderr, filePath, resolve) {
