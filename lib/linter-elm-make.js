...
export default {
  ...
  activate() {
    ...
    this.subscriptions.add(atom.commands.add('atom-text-editor', {
      ...
      'linter-elm-make:remove-unused-imports': this.removeUnusedImports.bind(this),
    ...
    this.subscriptions.add(atom.workspace.observeTextEditors((editor) => {
      ...
      self.subscriptions.add(editor.buffer.onWillSave(() => {
        once('lintCompleted', self.events, () => {
          if (atom.config.get('linter-elm-make.onSave.removeUnusedImports')) {
            const appliedFixes = self.removeUnusedImports();
  ...
  quickFixAll() {
    return this.execQuickFixes(this.allProblems());
  },
  removeUnusedImports() {
    return this.execQuickFixes(this.problemsWhere(fix => fix.type === 'Remove unused import'));
  },
  execQuickFixes(problems) {
    ...
    if (problems) {
      editor.transact(() => {
        problems.forEach(({range, fixes}) => {
          marker = editor.markBufferRange(range, {invalidate: 'never', persistent: false});
          marker.setProperties({fixes: fixes});
          markers.push(marker);
        });
        markers.forEach((marker) => {
          const fix = marker.getProperties().fixes[0];
          fixProblem(editor, marker.getBufferRange(), fix);
  ...
  problemsWhere(fixPredicate) {
    return this.allProblems()
      .map(problem => Object.assign({}, problem, { fixes: problem.fixes.filter(fixPredicate) }))
      .filter(problem => problem.fixes.length > 0);
  },
  toggleLintOnTheFly() {
  ...
  doLint(editorFilePath, projectDirectory, editor) {
    ...
    .then(result => {
      self.events.emit('lintCompleted');
      return result;
    });
  },
