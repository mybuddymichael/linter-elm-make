...
const SetMainPathView = require('./set-main-path-view');
...
module.exports = {
  config: {
    ...
    alwaysCompileMain: {
      ...
      description: 'Always compile the main file(s) instead of the active file.  The main file can be set using `Linter Elm Make: Set Main Path`.  If not set, the linter will look for `Main.elm` files in the source directories.  Modules unreachable from the main modules will not be linted.',
      type: 'boolean',
      default: false,
      order: 3
    },
  ...
  activate() {
    ...
    this.setMainPathView.onDidConfirm(({projectDirectory, mainPath}) => {
      const jsonFilePath = path.join(projectDirectory, 'elm-package.json');
      let json = fs.readJsonSync(jsonFilePath, {throws: false});
      if (json) {
        json['linter-elm-make'] = {mainPath: mainPath};
        fs.writeJsonSync(jsonFilePath, json);
        atom.notifications.addSuccess('Set `' + mainPath + '` as main path in `elm-package.json`', {});
    ...
    let subscribeToElmEditorEvents = (editor) => {
      ...
      editor.onDidDestroy(() => {
        if (atom.config.get('linter-elm-make.lintOnTheFly')) {
          // If editor was modified before it was destroyed, revert the contents of the associated compile file to the actual file's contents.
          if (editor.isModified()) {
            const editorFilePath = editor.getPath();
            if (editorFilePath) {
              const projectDirectory = lookupElmPackage(path.dirname(editorFilePath));
              const compileDirectory = self.compileDirectories[projectDirectory];
              if (compileDirectory) {
                const compileFilePath = path.join(compileDirectory, editorFilePath.replace(projectDirectory, ''));
                fs.writeFileSync(compileFilePath, fs.readFileSync(editorFilePath).toString());
              }
            }
          }
        }
    ...
    atom.workspace.observeActivePaneItem((item) => {
      if (atom.config.get('linter-elm-make.lintOnTheFly') &&
        ...
        if (self.prevElmEditor) {
          // When an editor loses focus, update the associated compile file.
          ...
          if (prevTextEditorPath) {
            ...
            if (projectDirectory) {
              const compileDirectory = self.compileDirectories[projectDirectory];
              if (compileDirectory) {
                const compileFilePath = path.join(compileDirectory, prevTextEditorPath.replace(projectDirectory, ''));
                fs.writeFileSync(compileFilePath, self.prevElmEditor.getText());
              }
            }
          }
        }
  ...
  deactivate() {
    ...
    this.compileDirectories = null;
    this.problems = null;
    this.quickFixes = null;
    this.quickFixView.destroy();
    this.setMainPathView.destroy();
    this.prevElmEditor = null;
  },
  quickFix() {
        ...
        if (json) {
          ...
          if (sourceDirectories) {
            ...
            sourceDirectories.forEach((sourceDirectory) => {
              fs.readdirSync(path.resolve(projectDirectory, sourceDirectory)).forEach((filename) => {
                if (path.extname(filename) === '.elm') {
                  paths.push(path.join(sourceDirectory, filename));
                }
              });
            });
            this.setMainPathView.show(editorFilePath.replace(projectDirectory + path.sep, ''), projectDirectory, paths);
          }
        }
      }
    }
  ...
  clearProjectBuildArtifacts() {
    ...
    .then(buildArtifactsDirectory => {
      if (buildArtifactsDirectory) {
        ...
        if (atom.config.get('linter-elm-make.lintOnTheFly')) {
          // If linting on the fly, also delete the build artifacts in the compile directory.
          if (projectDirectory) {
            const compileDirectory = self.compileDirectories[projectDirectory];
            if (compileDirectory) {
              const compileBuildArtifactsDirectory = buildArtifactsDirectory.replace(projectDirectory, compileDirectory);
              deleteBuildArtifacts(compileBuildArtifactsDirectory);
              if (atom.inDevMode()) {
                console.log('linter-elm-make: cleared compile directory build artifacts - ' + compileBuildArtifactsDirectory);
              }
            }
          }
        }
    ...
    const linter = {
      ...
      lint(editor) {
        ...
        if (editorFilePath) {
          ...
          if (atom.config.get('linter-elm-make.lintOnTheFly')) {
            // Lint on the fly.
            // If `compileDirectories` does not have an entry for `projectDirectory` yet...
            if (!self.compileDirectories[projectDirectory]) {
              // Create a temporary directory for the project.
              const compileDirectory = tmp.dirSync({prefix: 'linter-elm-make'}).name;
              if (atom.inDevMode()) {
                console.log('linter-elm-make: created temporary directory - ' + projectDirectory + ' -> ' + compileDirectory);
              }
              self.compileDirectories[projectDirectory] = compileDirectory;
              ...
              fs.copySync(projectDirectory, compileDirectory, {
              ...
              watcher.on('add', (filename) => {
                if (path.basename === 'elm-package.json' || path.extname(filename) === '.elm') {
                  ...
                  fs.copySync(filePath, path.join(compileDirectory, filename));
                }
              });
              ...
              // TODO When to close watcher, delete temporary directory, and delete self.compileDirectories[projectDirectory]?
              return self.compileInCompileDirectory(editorFilePath, compileDirectory, editor, projectDirectory);
            }
            // If `projectDirectory` already has an entry in `compileDirectories`...
            return self.compileInCompileDirectory(editorFilePath, self.compileDirectories[projectDirectory], editor, projectDirectory);
          } else {
            // Lint on save.
  ...
  provideGetCompileDirectory() {
    ...
    return (filePath) => {
      ...
      return self.compileDirectories[projectDirectory];
    };
  },
  compileInCompileDirectory(editorFilePath, compileDirectory, editor, projectDirectory) {
    const compileFilePath = path.join(compileDirectory, editorFilePath.replace(projectDirectory, ''));
    // Write contents of active editor to associated compile file.
    fs.writeFileSync(compileFilePath, editor.getText());
    if (atom.config.get('linter-elm-make.alwaysCompileMain')) {
      return this.compileMainFiles(editorFilePath, projectDirectory, compileDirectory, editor, (mainFilePath) => mainFilePath.replace(projectDirectory, compileDirectory));
    } else {
      // Compile active file.
      return this.executeElmMake(editorFilePath, compileFilePath, compileDirectory, editor, projectDirectory);
    }
  },
  ...
  getMainFilePaths(projectDirectory) {
    ...
    const mainPath = linterElmMake && linterElmMake.mainPath;
    const errorDetail =
      ...
      ' - Set the main path of the project using `Linter Elm Make: Set Main Path`. (Saves the main path to `elm-package.json`.)\n' +
      ' - Put a `Main.elm` file in at least one of the source directories.';
    // If `mainPath` exists, use that.
    if (mainPath) {
      const filePath = path.join(projectDirectory, mainPath);
  ...
  parseStdout(stdout, editorFilePath, inputFilePath, cwd, editor, projectDirectory) {
      ...
      if (!json) {
        ...
        return json.map((problem) => {
          ...
          if (cwd !== projectDirectory) {
            // problem.file is a compile file
    ...
    const problemsWithoutCompileFiles = problemsByLine.map((problems) => {
      return problems.filter((problem) => {
        // Filter out compile files.
        return problem.filePath.startsWith(projectDirectory);
      });
    });
    const allProblems = flattenArray(problemsWithoutCompileFiles);
  ...
  if (fs.existsSync(path.join(directory, 'elm-package.json'))) {
    ...
    if (parentDirectory === directory) {
      atom.notifications.addError('No `elm-package.json` beneath or above the edited file', {
        detail: 'You can generate an `elm-package.json` file by executing `elm package install` in the command line.',
        dismissable: true
      });
      return null;
    } else {
