      ...
      if (!json) {
        ...
        return json.map((problem) => {
          ...
          return {
            ...
            html: `<a class='${getAnchorForMessage(filePath, regionRange, subregionRange)}'></a>${colorize(_.escape(problem.overview))}<br/><br/>${colorize(_.escape(problem.details).split('\n').join('<br/>&nbsp;'))}`,
            filePath,
  ...
  switch (problem.tag) {
    case 'NAMING ERROR':
      ...
      if (matches && matches.length > 1) {
        ...
        return namingErrorHtml(suggestionFixes[0]);
      }
      matches = problem.details.match(/^The qualifier `(.+)` is not in scope\./);
      if (matches && matches.length > 1) {
        const suggestionFixes = (problem.suggestions || []).map((suggestion) => {
...
function namingErrorHtml(suggestion) {
  return {
    html: suggestion + ' ?'
  };
}
