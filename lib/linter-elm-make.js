...
export default {
  ...
  activate() {
    ...
    if (!atom.packages.isPackageLoaded('linter') && !atom.packages.isPackageLoaded('atom-ide-ui') && !atom.packages.isPackageLoaded('nuclide')) {
      atom.notifications.addError("A required package was not loaded", {
        detail: 'Please install or enable one of the following packages in your Settings view:\n' +
          ' - linter\n' +
          ' - atom-ide-ui\n' +
          ' - nuclide',
        dismissable: true
    ...
    this.subscriptions.add(atom.commands.add('atom-text-editor', {
      ...
      'linter-elm-make:clear-project-build-artifacts': this.clearProjectBuildArtifactsCommand.bind(this),
      'linter-elm-make:toggle-lint-on-the-fly': this.toggleLintOnTheFlyCommand,
      'linter-elm-make:toggle-always-compile-main': this.toggleAlwaysCompileMainCommand,
      'linter-elm-make:toggle-report-warnings': this.toggleReportWarningsCommand
    }));
    const self = this;
    ...
    return {
      ...
      getCodeActions: (editor, range, diagnostics) => {
        return new Promise((resolve) => {
          ...
          const fixesForRange = self.getFixesForRange(editor, range);
          if (!fixesForRange) {
            return resolve([]);
          }
  ...
  executeElmMake(editorFilePath, inputFilePath, cwd, editor, projectDirectory) {
    ...
    const timeoutConfig = parseFloat(atom.config.get('linter-elm-make.timeout'));
    const timeout = (isNaN(timeoutConfig) ? 10000 : (timeoutConfig === 0 ? Infinity : timeoutConfig * 1000));
  ...
  parseStderr(stderr, editorFilePath) {
    ...
    const problem = {
      type: 'error',
      html:
        '<span class="linter-elm-make-stderr">' +
  ...
  computeQuickFixesForEditor(editor) {
    ...
    if (problems) {
      const quickFixes =
        problems.map(({problem, range, regionRange, subregionRange}) => {
          return {
            fixes: quickFixing.getFixesForProblem(problem, range, regionRange, subregionRange, editor),
            range: range
          };
        })
        .filter(({fixes}) => {
