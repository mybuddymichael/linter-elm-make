...
export default {
  ...
  deactivate() {
    ...
    destroyAllMarkers(this.extraHintMarkers);
    this.extraHintMarkers = null;
  },
  clearElmEditorProblemsAndFixes(editor) {
    ...
    destroyEditorMarkers(this.extraHintMarkers, editor.id);
    this.extraHintMarkers[editor.id] = [];
  },
  quickFix() {
        ...
        if (fixesForCursorPosition) {
          return fixesForCursorPosition.fixes.map((fix) => {
            return {
              ...
              title: fix.text,
              selected: function() {
                fixProblem(textEditor, fixesForCursorPosition.range, fix);
  ...
  provideLinter() {
    ...
    this.subscriptions.add(atom.config.observe('linter-elm-make.showExtraHints', show => {
      if (!show) {
        destroyAllMarkers(self.extraHintMarkers);
        self.extraHintMarkers = {};
      }
    }));
    return linter;
  },
  ...
  copyProjectToWorkDirectory(projectDirectory, workDirectory, options) {
    ...
    return new Promise((resolve) => {
      ...
      setTimeout(() => {
        ...
        if (fileExists(elmStuffFilePath)) {
        ...
        if (sourceDirectories && sourceDirectories.length > 0) {
          ...
          parentSourceDirectories.forEach((projectSourceDirectory) => {
            ...
            if (fileExists(projectSourceDirectory)) {
              ...
              if (atom.config.get('linter-elm-make.logDebugMessages')) {
                if (fileExists(workSourceDirectory)) {
  ...
  getMainFilePaths(projectDirectory) {
    const elmPackageJsonFilePath = path.join(projectDirectory, 'elm-package.json');
    if (!fileExists(elmPackageJsonFilePath)) {
      return null;
    }
    ...
    if (fileExists(linterElmMakeJsonFilePath)) {
      ...
      if (sourceDirectories) {
        const mainFilePaths =
          ...
          .filter((mainFilePath) => {
            ...
            return fileExists(mainFilePath);
          });
  ...
  executeElmMake(editorFilePath, inputFilePath, cwd, editor, projectDirectory) {
    ...
    .then(data => {
      ...
      self.maybeShowExtraHints(editor);
    ...
    const problemsByLine = stdout.split('\n').map((line) => {
      ...
      if (!json) {
        ...
        return json.map((problem) => {
          const regionRange = regionToRange(problem.region);
          const subregionRange = regionToRange(problem.subregion);
          ...
          return {
            ...
            html:
              '<a class="' + getAnchorForMessage(filePath, regionRange, subregionRange) + '"></a>' +
              '<div class="linter-elm-make-problem-overview">' + _.escape(problem.overview) + '</div><br>' +
              _.escape(problem.details).split('\n').join('<br/>&nbsp;') +
              '<br>',
            filePath,
    ...
    return [
      {
        ...
        range: [[lineNumber, 0], [lineNumber, 0]] // TODO search for invalid import
      }
    ];
  },
  ...
  updateQuickFixesIndicatorDisplay() {
    if (this.quickFixesIndicator) {
      const fixesForCursorPosition = this.getFixesAtCursorPosition();
      this.quickFixesIndicator.item.textContent = fixesForCursorPosition ? 'Quick Fixes: ' + fixesForCursorPosition.fixes.length : '';
    }
  },
  ...
  maybeShowExtraHints(editor) {
    destroyEditorMarkers(this.extraHintMarkers, editor.id);
    this.extraHintMarkers[editor.id] = [];
    if (atom.config.get('linter-elm-make.showExtraHints')) {
      const problems = this.problems[editor.getPath()];
      if (problems) {
        problems.forEach((problem) => {
          const hint = getExtraHintForProblem(problem.problem, editor.getTextInBufferRange(problem.range));
          if (hint) {
            let element = document.createElement('div');
            element.classList.add('linter-elm-make-extra-hint');
            element.innerHTML = hint.html;
            // TODO: Look for a precise way to align the hint.
            const charSize = (atom.config.get('editor.fontSize') || 14) / 2;
            element.style.paddingLeft = (problem.range.start.column * charSize) + 'px';
            let marker = editor.markBufferPosition(problem.range.start);
            editor.decorateMarker(marker, {type: 'block', position: 'before', item: element});
            this.extraHintMarkers[editor.id].push(marker);
          }
        });
      }
    }
  ...
  maybeScrollProblemAtCursorIntoView() {
    if (atom.config.get('linter-elm-make.autoscrollIssueIntoView')) {
      const linterPanel = getLinterPanel();
      if (linterPanel) {
...
function lookupElmPackage(directory) {
  if (fileExists(path.join(directory, 'elm-package.json'))) {
    return directory;
  } else {
...
function getExtraHintForProblem(problem, rangeText) {
  let matches = null;
  switch (problem.tag) {
    case 'NAMING ERROR':
      matches = problem.details.match(/^No module called `(.+)` has been imported\./);
      if (matches && matches.length > 1) {
        const suggestionFixes = (problem.suggestions || []).map((suggestion) => {
          return rangeText.replace(matches[1], suggestion);
        });
        return namingErrorHtml(suggestionFixes, rangeText);
      }
      matches = problem.details.match(/^The qualifier `(.+)` is not in scope\./);
      if (matches && matches.length > 1) {
        const suggestionFixes = (problem.suggestions || []).map((suggestion) => {
      ...
      if (matches && matches.length > 1 && problem.suggestions && problem.suggestions.length > 0) {
        const suggestionFixes = problem.suggestions.map((suggestion) => {
          let rangeTextSegments = rangeText.split('.');
          rangeTextSegments.pop();
          return rangeTextSegments.join('.') + '.' + suggestion;
        });
        return namingErrorHtml(suggestionFixes, rangeText);
      }
      matches = problem.overview.match(/^Cannot find (?:variable|type|pattern) `(.+)`/);
      ...
      return null;
    case 'TYPE MISMATCH':
      matches = problem.details.match(/^The type annotation for `(.+)` says it always returns:\n\n((?:.|\n)+)\n\nBut the returned value \(shown above\) is a:\n\n((?:.|\n)+)/);
      if (matches && matches.length > 3) {
        // const name = matches[1];
        const expected = matches[2];
        const actual = matches[3].split('\n\n')[0]; // Discard the hints.
        return expectedAndActualTypeHtml(expected, actual);
      }
      matches = problem.details.match(/^Function `(.+)` is expecting the (.+) argument to be:\n\n((?:.|\n)+)\n\nBut it is:\n\n((?:.|\n)+)/);
      if (matches && matches.length > 4) {
        // const name = matches[1];
        // const argOrdinality = matches[2];
        const expected = matches[3];
        const actual = matches[4].split('\n\n')[0]; // Discard the hints.
        return expectedAndActualTypeHtml(expected, actual);
      }
      matches = problem.details.match(/^(.+) is expecting the left argument to be a:\n\n((?:.|\n)+)\n\nBut the left argument is:\n\n((?:.|\n)+)/);
      if (matches && matches.length > 3) {
        // const name = matches[1];
        const expected = matches[2];
        const actual = matches[3].split('\n\n')[0]; // Discard the hints.
      ...
      matches = problem.details.match(/^(.+) is expecting the right side to be a:\n\n((?:.|\n)+)\n\nBut the right side is:\n\n((?:.|\n)+)/);
      if (matches && matches.length > 3) {
        // const name = matches[1];
        const expected = matches[2];
        const actual = matches[3].split('\n\n')[0]; // Discard the hints.
      ...
      matches = problem.details.match(/^The `then` branch has type:\n\n((?:.|\n)+)\n\nBut the `else` branch is:\n\n((?:.|\n)+)/);
      if (matches && matches.length > 2) {
        const expected = matches[1];
        const actual = matches[2].split('\n\n')[0]; // Discard the hints.
        return expectedAndActualTypeHtml(expected, actual);
      }
      matches = problem.details.match(/^The (.+) branch has this type:\n\n((?:.|\n)+)\n\nBut the (.+) is:\n\n((?:.|\n)+)/);
      if (matches && matches.length > 4) {
      ...
      matches = problem.details.match(/^The pattern matches things of type:\n\n((?:.|\n)+)\n\nBut the values it will actually be trying to match are:\n\n((?:.|\n)+)$/);
      if (matches && matches.length > 2) {
      ...
      matches = problem.details.match(/The number definitely has this type:\n\n((?:.|\n)+)\n\nBut it is being used as:\n\n((?:.|\n)+)$/);
      if (matches && matches.length > 2) {
        const expected = matches[1];
        const actual = matches[2];
        return expectedAndActualTypeHtml(expected, actual);
      }
      return null;
    default:
...
function namingErrorHtml(suggestions, actual) {
  // const diff = jsdiff.diffChars(suggestion, actual);
  // let suggestionSpans = [];
  // let actualSpans = [];
  // diff.forEach((part) => {
  //   if (part.added) {
  //     actualSpans.push('<span class="linter-elm-make-diff-removed">' + _.escape(part.value) + '</span>');
  //   } else if (part.removed) {
  //     suggestionSpans.push('<span class="linter-elm-make-diff-added">' + _.escape(part.value) + '</span>');
  //   } else {
  //     suggestionSpans.push('<span>' + _.escape(part.value) + '</span>');
  //     actualSpans.push('<span>' + _.escape(part.value) + '</span>');
  //   }
  // });
  // return {
  //   html:
  //     '? ' + suggestionSpans.join('') + '<br>' +
  //     '✗ ' + actualSpans.join('')
  // };
  ...
  //       // Ignore.
  //     } else if (part.removed) {
  //       spans.push('<span class="linter-elm-make-diff-changed">' + _.escape(part.value) + '</span>');
  //     } else {
  //       spans.push('<span>' + _.escape(part.value) + '</span>');
  //     }
  //   });
  //   suggestionSpans.push('? ' + spans.join(''));
  // });
  // return {
  //   html: suggestionSpans.join('<br>')
  // };

  return {
    html:
      suggestions.map((suggestion) => {
        return _.escape(suggestion) + ' ?';
      }).join('<br>')
  };
}

function expectedAndActualTypeHtml(expected, actual) {
