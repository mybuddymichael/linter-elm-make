...
export default {
    ...
    this.subscriptions.add(atom.workspace.observeActivePaneItem((item) => {
      if (item && isElmEditor(item)) {
        if (!atom.config.get('linter-elm-make.showInferredTypeAnnotations')) {
          self.destroyTypeAnnotationMarkers(item.id);
        }
        if (atom.config.get('linter-elm-make.lintOnTheFly')) {
  ...
  deactivate() {
    ...
    Object.keys(this.typeAnnotationMarkers).forEach((editorId) => {
      this.destroyTypeAnnotationMarkers(editorId);
    });
  ...
  provideLinter() {
    ...
    this.subscriptions.add(atom.config.observe('linter-elm-make.showInferredTypeAnnotations', show => {
      if (!show) {
        const editor = atom.workspace.getActiveTextEditor();
        if (isElmEditor(editor)) {
          self.destroyTypeAnnotationMarkers(editor.id);
        }
      }
    }));
    return linter;
  ...
  destroyTypeAnnotationMarkers(editorId) {
    let markers = this.typeAnnotationMarkers[editorId];
    if (markers) {
      markers.forEach((marker) => {
        marker.destroy();
      });
  ...
  maybeShowInferredTypeAnnotations(editor) {
    ...
    this.destroyTypeAnnotationMarkers(editor.id);
    ...
    if (atom.config.get('linter-elm-make.showInferredTypeAnnotations')) {
      this.quickFixesWhere(editor, fix => fix.type === 'Add type annotation')
        .forEach(({range, fixes}) => {
          ...
          let marker = editor.markScreenPosition(range.start);
...
function showNoQuickFixesFound() {
  const detail = atom.config.get('linter-elm-make.lintOnTheFly') ? '' : 'You might need to lint again.';
