...
module.exports = {
  ...
  activate() {
    ...
    atom.commands.add('atom-text-editor', {
      ...
      'linter-elm-make:toggle-ignore-warnings': this.toggleIgnoreWarnings
    });
    const self = this;
    ...
    let subscribeToElmEditorEvents = (editor) => {
        ...
        if (!atom.config.get('linter-elm-make.onlyLintOnSave')) {
          ...
          if (editor.isModified()) {
            ...
            if (editorFilePath) {
              ...
              const tempLintDirectory = self.tempLintDirectories[projectDirectory];
      ...
      // TODO When do we delete all of a project's entries in `this.problems`?  When the last editor for the project was closed?
    };
  ...
  toggleIgnoreWarnings() {
    const newValue = toggleConfig('linter-elm-make.ignoreWarnings');
    atom.notifications.addInfo('"Ignore Warnings" is now ' + (newValue ? 'ON' : 'OFF'), {});
      ...
      if (buildArtifactsDirectory) {
        ...
        if (!atom.config.get('linter-elm-make.onlyLintOnSave')) {
          ...
          if (projectDirectory) {
            const tempLintDirectory = self.tempLintDirectories[projectDirectory];
            if (tempLintDirectory) {
              deleteBuildArtifacts(buildArtifactsDirectory.replace(projectDirectory, tempLintDirectory));
            }
          }
        }
        ...
        if (editorFilePath) {
          ...
          if (atom.config.get('linter-elm-make.onlyLintOnSave')) {
            ...
            if (atom.config.get('linter-elm-make.alwaysCompileMain')) {
              // Compile `Main.elm`.
              const mainFilePath = self.getMainFilePath(projectDirectory);
              if (mainFilePath) {
                return self.executeElmMake(editorFilePath, mainFilePath, projectDirectory, editor, projectDirectory);
              } else {
                return [];
              }
            } else {
            ...
            // On-the-fly linting.
            // If `tempLintDirectories` does not have an entry for `projectDirectory` yet...
            if (!self.tempLintDirectories[projectDirectory]) {
              ...
              self.tempLintDirectories[projectDirectory] = tempLintDirectory;
              ...
              // TODO When to close watcher, delete temporary directory, and delete self.tempLintDirectories[projectDirectory]?  When the last editor for the project was closed?
              return self.compileInTempDirectory(editorFilePath, tempLintDirectory, editor, projectDirectory);
            }
            // If `projectDirectory` already has an entry in `tempLintDirectories`...
            return self.compileInTempDirectory(editorFilePath, self.tempLintDirectories[projectDirectory], editor, projectDirectory);
          }
        }
        return [];
      }
  ...
  getMainFilePath(projectDirectory) {
    let json = (() => {
      try {
    ...
    if (sourceDirectories) {
      ...
      const mainFilePath = path.join(projectDirectory, sourceDirectory, 'Main.elm');
  ...
  compileInTempDirectory(editorFilePath, tempLintDirectory, editor, projectDirectory) {
    ...
    if (atom.config.get('linter-elm-make.alwaysCompileMain')) {
      // Compile `Main.elm`.
      const mainFilePath = this.getMainFilePath(projectDirectory);
      if (mainFilePath) {
        return this.executeElmMake(editorFilePath, mainFilePath.replace(projectDirectory, tempLintDirectory), tempLintDirectory, editor, projectDirectory);
      } else {
        return [];
      }
    } else {
