    ...
    if (problems) {
      const quickFixes =
        problems.map(({problem, range}) => {
          return {
            fixes: quickFixing.getFixesForProblem(problem, editor.getTextInBufferRange(range), editorFilePath),
            range: range
          };
        })
        .filter(({fixes}) => {
