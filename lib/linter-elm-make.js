...
export default {
  ...
  activate() {
    if (!atom.packages.getLoadedPackage('language-elm')) {
      atom.notifications.addError("The `language-elm` package wasn't found", {
        detail: 'Please install the `language-elm` package in your Settings view.',
        dismissable: true
      });
    }
    if (!atom.packages.getLoadedPackage('linter')) {
      atom.notifications.addError("The `linter` package wasn't found", {
    ...
    var onDidChangeCursorPositionDebouncer = null;
    let subscribeToElmEditorEvents = (editor) => {
      self.subscriptions.add(editor.onDidChangeCursorPosition(() => {
        if (onDidChangeCursorPositionDebouncer) {
          clearTimeout(onDidChangeCursorPositionDebouncer);
        }
        onDidChangeCursorPositionDebouncer =
          setTimeout(() => {
            self.updateLinterUI();
          }, 150);
      }));
    ...
    this.subscriptions.add(atom.workspace.observeActivePaneItem((item) => {
      if (item && isElmEditor(item)) {
        ...
        self.scrollProblemAtCursorIntoView();
      } else {
        self.prevElmEditor = null;
      }
    }));
      ...
      .then(buildArtifactsDirectory => {
        if (buildArtifactsDirectory) {
          ...
          if (buildArtifactsDirectoryCleared && atom.inDevMode()) {
            devLog('Cleared project directory build artifacts - ' + buildArtifactsDirectory);
          }
          if (atom.config.get('linter-elm-make.lintOnTheFly')) {
            ...
            if (projectDirectory) {
              ...
              if (workDirectory) {
                ...
                if (workDirectoryBuildArtifactsDirectoryCleared && atom.inDevMode()) {
                  devLog('Cleared work directory build artifacts - ' + workDirectoryBuildArtifactsDirectory);
                }
              }
            }
          }
      ...
      if (configWorkDirectory && configWorkDirectory.trim() !== '') {
        if (!self.workDirectories[projectDirectory]) {
          ...
          if (!fs.existsSync(workDirectory)) {
            devLog('Created work directory - ' + projectDirectory + ' -> ' + workDirectory);
            const self = this;
        ...
        .then(() => {
          if (atom.config.get('linter-elm-make.lintOnTheFly')) {
            ...
            if (!self.workDirectories[projectDirectory]) {
              ...
              devLog('Created temporary work directory - ' + workDirectory + ' -> ' + projectDirectory);
              self.workDirectories[projectDirectory] = workDirectory;
              self.copyProjectToWorkDirectory(projectDirectory, workDirectory)
                .then(() => {
                  self.watchProjectDirectory(projectDirectory, workDirectory);
  ...
  copyProjectToWorkDirectory(projectDirectory, workDirectory, options) {
    ...
    return new Promise((resolve) => {
      ...
      devLog('Copying project directory to work directory - ' + projectDirectory + ' -> ' + workDirectory);
      ...
      setTimeout(() => {
            ...
            if (fs.existsSync(projectSourceDirectory)) {
              ...
              if (atom.inDevMode()) {
                if (fs.existsSync(workSourceDirectory)) {
                  devLog('Copied project source directory to work directory - ' + projectSourceDirectory + ' -> ' + workSourceDirectory);
                }
              }
            }
          });
        ...
        devLog('Copied project directory to work directory - ' + projectDirectory + ' -> ' + workDirectory);
        // setTimeout(() => {
        //   forceLintActiveElmEditor();
        // }, 300);
        return resolve();
    ...
    if (json) {
      ...
      if (sourceDirectories) {
        sourceDirectories.forEach((sourceDirectory) => {
          ...
          devLog('Deleted source directory in work directory - ' + workDirectorySourceDirectory);
        });
      }
    }
  },
  ...
  executeElmMake(editorFilePath, inputFilePath, cwd, editor, projectDirectory) {
    ...
    const progressIndicator = this.statusBar.addLeftTile({
    ...
    const problemsByLine = stdout.split('\n').map((line) => {
      ...
      if (!json) {
        ...
        return json.map((problem) => {
          ...
          // HACK: Add an anchor so that we can scroll the message into view when the cursor position changes.
          return {
            type: problem.type,
            html: `<a class='${getAnchorForMessage(filePath, range)}'></a>${colorize(problem.overview)}<br/><br/>${colorize(problem.details.split('\n').join('<br/>&nbsp;'))}`,
            filePath: filePath,
            range: range,
            problem: problem
          };
  ...
  scrollProblemAtCursorIntoView() {
    var linterPanel = document.getElementsByTagName('linter-panel');
    if (linterPanel && linterPanel.length > 0) {
      ...
      if (problem) {
        var problemAnchor = linterPanel.getElementsByClassName(getAnchorForMessage(problem.filePath, problem.range));
  ...
  getProblemAtCursorPosition() {
    ...
    if (problems) {
      const BreakException = {};
      try {
        problems.forEach(({range, filePath}) => {
          if (range.containsPoint(position)) {
            // Problem found! Get out of loop.
