...
module.exports = {
  ...
  activate() {
      ...
      editor.onDidDestroy(() => {
        if (!atom.config.get('linter-elm-make.onlyLintOnSave')) {
          ...
          if (editor.isModified()) {
            ...
            if (editorFilePath) {
              const projectRootDirectory = lookupElmPackage(path.dirname(editorFilePath));
              const tempLintDirectory = self.tempLintDirectories[projectRootDirectory];
              if (tempLintDirectory) {
                const tempFilePath = path.join(tempLintDirectory, editorFilePath.replace(projectRootDirectory, ''));
    ...
    atom.workspace.observeTextEditors((editor) => {
      ...
      if (path.extname(editor.getPath()) === '.elm') {
    ...
    this.prevElmTextEditor = null;
    atom.workspace.observeActivePaneItem((item) => {
      if (item && item == atom.workspace.getActiveTextEditor() &&
        path.extname(item.getPath()) === '.elm' &&
        !atom.config.get('linter-elm-make:onlyLintOnSave')) {
        if (self.prevElmTextEditor) {
          // When a text editor loses focus, update its corresponding temporary file.
          const prevTextEditorPath = self.prevElmTextEditor.getPath();
          if (prevTextEditorPath) {
            const projectRootDirectory = lookupElmPackage(path.dirname(prevTextEditorPath));
            if (projectRootDirectory) {
              const tempLintDirectory = self.tempLintDirectories[projectRootDirectory];
              if (tempLintDirectory) {
                const tempFilePath = path.join(tempLintDirectory, prevTextEditorPath.replace(projectRootDirectory, ''));
                fs.writeFileSync(tempFilePath, self.prevElmTextEditor.getText());
              }
            }
          }
        ...
        self.prevElmTextEditor = item;
      } else {
        self.prevElmTextEditor = null;
      }
    });
  },
  forceLint(editor) {
      ...
      if (buildArtifactsDirectory) {
        ...
        if (!atom.config.get('linter-elm-make.onlyLintOnSave')) {
          ...
          if (projectRootDirectory) {
            const tempLintDirectory = self.tempLintDirectories[projectRootDirectory];
            if (tempLintDirectory) {
              deleteBuildArtifacts(buildArtifactsDirectory.replace(projectRootDirectory, tempLintDirectory));
            }
          }
        }
        atom.notifications.addInfo('Cleared ' + buildArtifactsDirectory, {});
      }
    });
  },
  provideLinter() {
        ...
        if (editorFilePath) {
          ...
          if (atom.config.get('linter-elm-make.onlyLintOnSave')) {
            ...
            if (atom.config.get('linter-elm-make.alwaysCompileMain')) {
              // Compile `Main.elm`.
              const mainFilePath = self.getMainFilePath(projectRootDirectory);
              if (mainFilePath) {
                return self.executeElmMake(editorFilePath, mainFilePath, projectRootDirectory, editor, projectRootDirectory);
              } else {
                return [];
              }
            } else {
              // Compile active file.
              return self.executeElmMake(editorFilePath, editorFilePath, projectRootDirectory, editor, projectRootDirectory);
            }
          } else {
            // On-the-fly linting.
            // // Create and compile a temporary file containing the text in the editor.
            // return helpers.tempFile(path.basename(editorFilePath), editor.getText(), (tempFilePath) => {
            //   return self.executeElmMake(editorFilePath, tempFilePath, projectRootDirectory, editor);
            // });

            // If `tempLintDirectories` does not have an entry for `projectRootDirectory` yet...
            if (!self.tempLintDirectories[projectRootDirectory]) {
              // Create a temporary directory for the project.
              const tempLintDirectory = tmp.dirSync({prefix: 'linter-elm-make'}).name;
              if (atom.inDevMode()) {
                console.log('linter-elm-make: created temporary directory - ' + projectRootDirectory + ' -> ' + tempLintDirectory);
              }
              self.tempLintDirectories[projectRootDirectory] = tempLintDirectory;
              getProjectBuildArtifactsDirectory(editorFilePath)
              .then(buildDirectory => {
                // Recursively copy `.elm` files, `elm-package.json`, and `elm-stuff` from project directory to temporary directory.
                // Does not copy `*.elmi` and `*.elmo` files of the project's build artifacts (e.g. `elm-stuff/build-artifacts/0.17.0/user/project/1.0.0`).
                fs.copySync(projectRootDirectory, tempLintDirectory, {
                  // preserveTimestamps: true,
                  filter: (filePath) => {
                    return (path.dirname(filePath).replace(projectRootDirectory, '').startsWith(path.sep + 'elm-stuff' + path.sep) ||
                            path.basename(filePath) === 'elm-package.json' ||
                            path.extname(filePath) === '.elm') &&
                           (!buildDirectory || !filePath.startsWith(buildDirectory));
                  }
                });
                // TODO Do not copy folders without `.elm` files (`fs-extra`-related).
                ...
                watcher.on('unlink', (filename) => {
                  if (atom.inDevMode()) {
                    console.log('linter-elm-make: unlink detected - ', path.join(projectRootDirectory, filename));
                  }
                  fs.removeSync(path.join(tempLintDirectory, filename));
                });
                watcher.on('unlinkDir', (dirname) => {
                  if (atom.inDevMode()) {
                    console.log('linter-elm-make: unlinkDir detected - ', path.join(projectRootDirectory, dirname));
                  }
                ...
                watcher.on('change', (filename) => {
                  if (path.basename === 'elm-package.json') {
                    ...
                    const editor = atom.workspace.getActiveTextEditor();
                    const editorFilePath = editor.getPath();
                    if (editorFilePath && path.extname(editorFilePath) === '.elm') {
                      self.forceLint(editor);
                    }
                  }
                });
                // // //
                watcher.on('all', (type, event) => {
                  console.log(type, event);
                });
                // TODO When to close watcher, delete temporary directory, and delete self.tempLintDirectories[projectRootDirectory]?
                return self.compileInTempDirectory(editorFilePath, tempLintDirectory, editor, projectRootDirectory);
              });
            }
            // If `projectRootDirectory` already has an entry in `tempLintDirectories`...
            return self.compileInTempDirectory(editorFilePath, self.tempLintDirectories[projectRootDirectory], editor, projectRootDirectory);
          }
        }
        return [];
      }
  ...
  getMainFilePath(projectRootDirectory) {
    let json = (() => {
      try {
  ...
  compileInTempDirectory(editorFilePath, tempLintDirectory, editor, projectRootDirectory) {
    const tempFilePath = path.join(tempLintDirectory, editorFilePath.replace(projectRootDirectory, ''));
    // Write contents of active editor to associated temporary file.
    fs.writeFileSync(tempFilePath, editor.getText());
    if (atom.config.get('linter-elm-make.alwaysCompileMain')) {
      // Compile `Main.elm`.
      const mainFilePath = this.getMainFilePath(projectRootDirectory);
      if (mainFilePath) {
        return this.executeElmMake(editorFilePath, mainFilePath.replace(projectRootDirectory, tempLintDirectory), tempLintDirectory, editor, projectRootDirectory);
      } else {
        return [];
      }
    } else {
      // Compile active file.
      return this.executeElmMake(editorFilePath, tempFilePath, tempLintDirectory, editor, projectRootDirectory);
    }
  },
  consumeStatusBar(statusBar) {
    module.statusBar = statusBar;
  },
  executeElmMake(editorFilePath, inputFilePath, cwd, editor, projectRootDirectory) {
    console.log('executeElmMake', editorFilePath, inputFilePath); // // //
    ...
    .then(data => {
      let result;
      if (data.stderr === '') {
        result = self.parseStdout(data.stdout, editorFilePath, inputFilePath, cwd, editor, projectRootDirectory);
      } else {
  ...
  parseStdout(stdout, editorFilePath, inputFilePath, cwd, editor, projectRootDirectory) {
    const problemsByLine = stdout.split('\n').map((line) => {
      ...
      if (!json) {
        ...
        return json.map((problem) => {
          ...
          if (cwd !== projectRootDirectory) {
            // problem.file is a temporary file
            filePath = filePath.replace(cwd, projectRootDirectory);
          }
          return {
            type: problem.type,
    ...
    .then(data => {
      ...
      if (json) {
        if (json.repository && json.version) {
          ...
          if (user && project) {
            return path.join(projectRootDirectory, 'elm-stuff', 'build-artifacts', elmPlatformVersion, user, project, json.version);
          } else {
