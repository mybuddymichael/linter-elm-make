...
function doLint(filePath, inputFilePath, textEditor) {
  ...
  const cwd = lookupElmPackage(path.dirname(filePath));
  if (cwd === null) {
  ...
  .then(data => {
    return new Promise((resolve, reject) => {
      if (data.stderr === '') {
        parseStdout(data.stdout, filePath, cwd, textEditor, progressIndicator, resolve);
      } else {
        parseStderr(data.stderr, filePath, progressIndicator, resolve);
      }
    });
  })
  .catch(errorMessage => {
...
function parseStdout(stdout, filePath, cwd, textEditor, progressIndicator, resolve) {
    ...
    if (!json) {
      ...
      return json.map((problem) => {
        ...
        return {
          ...
          filePath: !atom.config.get('linter-elm-make.onlyLintOnSave') ? filePath : ((problem.file && path.join(cwd, path.normalize(problem.file))) || filePath),
          range: range,
...
function parseStderr(stderr, filePath, progressIndicator, resolve) {
