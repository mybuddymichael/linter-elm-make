...
import oldFormatting from './old-formatting';
        ...
        if (fixesForCursorPosition) {
          return fixesForCursorPosition.fixes.map(fix => {
            return {
              ...
              class:
                'linter-elm-make-fix--' +
                fix.type.toLowerCase().replace(/ /g, '-'),
              title: fix.text,
              selected: function() {
                quickFixing.fixProblem(
                  textEditor,
    ...
    const problemsByLine = stdout.split('\n').map(line => {
      ...
      return json.map(problem => {
        ...
        return {
          type: problem.type,
          html:
            '<div class="linter-elm-make-problem-overview">' +
            _.escape(problem.overview) +
            '</div><br>' +
            oldFormatting.formatProblemDetails(problem) +
