...
export default {
  // TODO: Tests.
  getFixesForProblem(problem, range, regionRange, subregionRange, editor) {
    if (problem.tag) {
      return getFixesForTaggedProblem(problem, range, regionRange, subregionRange, editor);
    }
    let matches = null;
...
function getFixesForTaggedProblem(problem, range, regionRange, subregionRange, editor) {
  let matches = null;
  switch (problem.tag) {
    case 'NAMING ERROR':
    ...
    case 'TYPE MISMATCH':
      ...
      if (problem.details === "(+) is expecting the left argument to be a:\n\n    number\n\nBut the left argument is:\n\n    String\n\nHint: To append strings in Elm, you need to use the (++) operator, not (+).\n<http://package.elm-lang.org/packages/elm-lang/core/latest/Basics#++>") {
        ...
        let plusRange = null;
        editor.scanInBufferRange(/\+/, [subregionRange.end, regionRange.end], ({range, stop}) => {
          plusRange = range;
          stop();
        });
        if (plusRange) {
          const textBeforePlus = getTextInRange(editor, new Range(regionRange.start, plusRange.start));
          const textAfterPlus = getTextInRange(editor, new Range(plusRange.end, regionRange.end));
          return [{
            type: 'Replace with',
      ...
      if (matches && matches.length > 1) {
        return [{
          type: 'Replace with',
          text: '(toString <| ' +  getTextInRange(editor, subregionRange) + ')'
        }];
      }
