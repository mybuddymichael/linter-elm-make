      ...
      case 'Define top-level':
        if (fix.filePath) {
          ...
          editor.transact(() => {
            ...
            editor.setCursorBufferPosition([
              topLevelEnd.row + (atEndOfFile ? 3 : 2),
              fix.kind === 'type' ? 6 : 4,
            ]);
          });
        }
  ...
  switch (problem.tag) {
    case 'NAMING ERROR':
      ...
      if (matches && matches.length > 2) {
        ...
        const filePath = path.join(projectDirectory, moduleName + '.elm');
      ...
      if (matches && matches.length > 2) {
        ...
        const isLowerCase =
          name.length > 0 && name[0] === name[0].toLowerCase();
        const defineTopLevelFix =
          kind === 'type' || (kind === 'variable' && isLowerCase)
            ? [
                {
                  type: 'Define top-level',
    ...
    case 'TYPE MISMATCH':
        ...
        return [
          {
            type: 'Replace with',
            text:
              '(toString <| ' + getTextInRange(editor, subregionRange) + ')',
          },
        ];
      }
      ...
      matches = problem.overview.match(
        /^Function `(.+)` is expecting (\d+) argument, but was given (\d+)\.$/
      );
      ...
      if (matches && matches.length > 3) {
        // NOTE: This should only change the return type (e.g. in `Model -> Cmd Msg`, should only change `Cmd Msg`)
        const name = matches[1];
