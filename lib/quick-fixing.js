...
export default {
  ...
  getFixesForProblem(
    ...
    subregionRange,
    editor,
    getSymbolsMatchingTypeFunction
  ) {
    if (problem.tag) {
  ...
  fixProblem(
    ...
    fix,
    getSymbolsMatchingTypeFunction,
    showSymbolsMatchingTypeFunction
  ) {
    switch (fix.type) {
      case 'Replace with':
      ...
      case 'Search for symbols matching type':
        if (getSymbolsMatchingTypeFunction) {
          const projectDirectory = helper.lookupElmPackage(
          ...
          getSymbolsMatchingTypeFunction(
            ...
            showSymbolsMatchingTypeFunction(editor, range, symbols);
          });
        }
        break;
    }
    ...
    case 'TYPE MISMATCH':
      ...
      if (matches && matches.length > 3) {
        ...
        if (oldTypeAnnotation) {
          ...
          if (getSymbolsMatchingTypeFunction) {
            const typeToMatch = newReturnType + ' -> ' + matches[2];
            fixes.push({
              type: 'Search for symbols matching type',
              text: typeToMatch,
