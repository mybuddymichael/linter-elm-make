...
export default {
  ...
  lookupElmPackage(directory, editorFilePath) {
    atom.project.getPaths().forEach(projectPath => {
      if (
        editorFilePath.startsWith(projectPath + path.sep) &&
        this.fileExists(path.join(projectPath, 'elm-package.json'))
      ) {
        return projectPath;
      }
    });
    return this.lookupElmPackageRecur(directory, editorFilePath);
  },

  lookupElmPackageRecur(directory, editorFilePath) {
    if (this.fileExists(path.join(directory, 'elm-package.json'))) {
      ...
      if (parentDirectory === directory) {
        ...
        return this.lookupElmPackageRecur(parentDirectory, editorFilePath);
      }
    }
  },
