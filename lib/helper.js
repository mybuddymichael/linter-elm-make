...
export default {
  ...
  lookupElmPackage(directory, editorFilePath) {
    let projectPaths = _.sortBy(atom.project.getPaths(), projectPath => {
      return projectPath.length;
    });
    let shortestProjectPath = null;
    projectPaths.forEach(projectPath => {
      if (containsPath(editorFilePath, projectPath + path.sep)) {
        shortestProjectPath = projectPath;
      }
    });
    return this.lookupElmPackageRecur(
      directory,
      editorFilePath,
      shortestProjectPath
    );
  ...
  lookupElmPackageRecur(directory, editorFilePath, shortestProjectPath) {
    if (this.fileExists(path.join(directory, 'elm-package.json'))) {
      return directory;
    } else {
      const parentDirectory = path.join(directory, '..');
      // Stop when we're already at the root of the file system or the Atom project.
      if (
        parentDirectory === directory ||
        !containsPath(parentDirectory, shortestProjectPath + path.sep)
      ) {
