...
function formatTypeParts(rawParts, props) {
  ...
  return parts.map((part, index) => {
    ...
    return parts2.map((part2, index2) => {
      const maybeLineBreak = index2 === lastIndex2 ? '' : <br />;
      if (isAType(part2.replace(/,$/, ''))) {
        return (
          <span>
            {formatTypePart(part2, props)}
            {maybeSpace}
            {maybeLineBreak}
          </span>
        );
