  ...
  formatProblemDetails(problem) {
    ...
    switch (problem.tag) {
      case 'TYPE MISMATCH':
        ...
        if (matches && matches.length > 3) {
          ...
          return problem.details.replace(regex,
            ...
            'Which does not contain a field named `' + formatActualName('', field) + '`.' +
            formatHints(hints));
        }
      ...
      case 'PORT ERROR':
        ...
        if (matches && matches.length > 3) {
          const actualType = matches[2];
          return problem.details.replace(regex, '$1' + formatActualType('', actualType) + '$3');
        }
        return formatDefault(problem.details);
      case 'BAD RECURSION':
...
function formatHint(hint) {
  ...
  if (matches && matches.length > 3) {
    const name = matches[2];
    return hint.replace(regex, '$1' + formatExpectedName(name, '') + '$3');
  }
  regex = /^(Looks like a record is missing these fields: )((?:.|\n)+)$/;
  matches = hint.match(regex);
  if (matches && matches.length > 2) {
    const formatFunction = (field) => {
      return formatExpectedName(field, '');
    };
  ...
  if (matches && matches.length > 3) {
    ...
    return hint.replace(regex, '$1' + formatExpectedName(numArgsNeeded, '') + '$3');
  }
