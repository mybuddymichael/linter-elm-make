  ...
  switch (problem.tag) {
    case 'TYPE MISMATCH':
      ...
      if (matches && matches.length > 4) {
        ...
        const actualType = hints.shift();
        return problem.details.replace(regex,
          'Function `' + emphasize(name) + '` is expecting the ' + emphasize(argOrdinality) + ' argument to be:\n\n' +
          formatExpectedType(expectedType, actualType) + '\n\n' +
          'But it is:\n\n' +
          formatActualType(expectedType, actualType) +
          formatHints(hints));
      ...
      if (matches && matches.length > 3) {
        ...
        const actualType = hints.shift();
        return problem.details.replace(regex,
          'Function `' + emphasize(name) + '` is expecting the argument to be:\n\n' +
          formatExpectedType(expectedType, actualType) + '\n\n' +
          'But it is:\n\n' +
          formatActualType(expectedType, actualType) +
          formatHints(hints));
    ...
    case 'NAMING ERROR':
      ...
      if (matches && matches.length > 6) {
        ...
        const suggestions = matches[6];
        return problem.details.replace(regex, '$1' + formatInfo(moduleName) + '$3' + formatIncorrectName(unexposed) + '$5' + emphasize(suggestions));
      }
      return formatDefault(problem.details);
    case 'SYNTAX PROBLEM':
    ...
    case 'PORT ERROR':
      ...
      if (matches && matches.length > 2) {
        ...
        return problem.details.replace(regex,
          ...
          formatIncorrectType(actualType) + '\n\n' +
          'But you need to use the particular format described here:\n<a href="' + url + '">' + url + '</a>');
      }
    ...
    case 'BAD RECURSION':
      ...
      if (matches && matches.length > 2) {
        const url = _.escape(matches[2]);
        return problem.details.replace(regex, '$1<a href="' + url + '">' + url + '</a>');
      }
      return formatDefault(problem.details);
    default:
      return formatDefault(problem.details);
...
function formatIncorrectType(type) {
  return '<div class="linter-elm-make-details-actual-type"><span class="linter-elm-make-actual-type-diff-changed">' + _.escape(type) + '</span></span>';
}

function formatCorrectName(name) {
  return '<span class="linter-elm-make-details-expected-name"><span class="linter-elm-make-expected-name-diff-changed">' + _.escape(name) + '</span></span>';
}

function formatIncorrectName(name) {
  return '<span class="linter-elm-make-details-actual-name"><span class="linter-elm-make-actual-name-diff-changed">' + _.escape(name) + '</span></span>';
}
