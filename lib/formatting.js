  ...
  switch (problem.tag) {
    case 'TYPE MISMATCH':
      ...
      if (matches && matches.length > 4) {
        ...
        return problem.details.replace(regex,
          '$1' +
          formatActualType(expectedType, actualType) + '$3' +
          formatExpectedType(expectedType, actualType) +
          formatHints(hints));
      }
      ...
      if (matches && matches.length > 4) {
        ...
        return problem.details.replace(regex,
          '$1' +
          formatExpectedType(expectedType, actualType) + '$3' +
          formatActualType(expectedType, actualType) +
          formatHints(hints));
      }
      ...
      matches = problem.details.match(regex);
      if (matches && matches.length > 8) {
        const record = matches[2];
        const actualType = matches[4];
        const field = matches[6];
        const hints = matches[8].split('\n\n');
        return problem.details.replace(regex,
          '$1' + emphasize(record) + '$3' +
          formatActualType(actualType, actualType) + '$5' +
          formatIncorrectName(field) + '$7' +
          formatHints(hints));
      }
