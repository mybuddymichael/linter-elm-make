...
export default {
  formatProblemDetails(problem) {
    return this.formatProblemDetails1(problem).replace(/\n/g, '<br>');
  },
  formatProblemDetails1(problem) {
    ...
    if (matches && matches.length > 7) {
      ...
      return problem.replace(
        ...
        '$1' +
          formatInfo(filePath) +
          '$3' +
          formatCorrectName(expectedModuleName) +
          '$5' +
          formatIncorrectName(actualModuleName) +
          '$7'
      );
    }
    ...
    if (matches && matches.length > 5) {
      ...
      return problem.replace(
        regex,
        '$1' +
          formatIncorrectName(notFoundModuleName) +
          '$3' +
          formatInfo(usingModuleName) +
          '$5'
      );
    }
    // TODO:
  ...
  switch (problem.tag) {
    case 'TYPE MISMATCH':
      ...
      if (matches && matches.length > 3) {
        ...
        return problem.details.replace(
          regex,
      ...
      if (matches && matches.length > 4) {
        ...
        return problem.details.replace(
          regex,
          'The ' +
            emphasize(branch1Ordinality) +
            ' branch has this type:\n\n' +
            formatExpectedType(expectedType, actualType) +
            '\n\n' +
            'But the ' +
            emphasize(branch2Ordinality) +
            ' is:\n\n' +
            formatActualType(expectedType, actualType) +
            formatHints(hints)
        );
      ...
      if (matches && matches.length > 8) {
        ...
        return problem.details.replace(
          regex,
          '$1' +
            emphasize(recordName) +
            '$3' +
            formatActualType(actualType, actualType) +
            '$5' +
            formatIncorrectName(field) +
            '$7' +
            formatHints(hints)
        );
      }
    ...
    case 'NAMING ERROR':
      ...
      if (matches && matches.length > 7) {
        ...
        return problem.details.replace(
          regex,
          '$1' +
            formatInfo(moduleName) +
            '$3' +
            formatIncorrectName(unexposed) +
            '$5$6' +
    ...
    case 'MISSING PATTERNS':
      ...
      if (matches && matches.length > 9) {
        ...
        return problem.details.replace(
          regex,
          '$1' +
            formatIncorrectType(missingPatterns) +
            '$3' +
            formatUrl(url) +
            '$5' +
            formatInfo(wildcard) +
            '$7' +
            formatInfo(debugCrash) +
            '$9'
        );
      }
      return formatDefault(problem.details);
    ...
    case 'PORT ERROR':
      ...
      if (matches && matches.length > 3) {
        ...
        return problem.details.replace(
          regex,
          '$1' + formatIncorrectType(actualType) + '$3' + formatUrl(url)
        );
      }
      ...
      if (matches && matches.length > 3) {
        ...
        return problem.details.replace(
          regex,
          '$1' + formatIncorrectType(actualType) + '$3'
        );
      }
      return formatDefault(problem.details);
...
function emphasize(text) {
  return (
    '<span class="linter-elm-make-details-emphasized">' +
    _.escape(text) +
    '</span>'
  );
}
...
function formatInfo(text) {
  return (
    '<span class="linter-elm-make-details-info">' + _.escape(text) + '</span>'
  );
}
...
function formatActualType(expected, actual) {
  ...
  return (
    '<div class="linter-elm-make-details-actual-type">' +
    diff
      .map(part => {
        if (part.added) {
          return (
            '<span class="linter-elm-make-actual-type-diff-changed">' +
            _.escape(part.value) +
            '</span>'
          );
        } else if (!part.removed) {
...
function formatExpectedName(expected, actual) {
  ...
  return (
    '<span class="linter-elm-make-details-expected-name">' +
    diff
      .map(part => {
        if (part.added) {
          return (
            '<span class="linter-elm-make-expected-name-diff-changed">' +
            _.escape(part.value) +
            '</span>'
          );
        } else if (!part.removed) {
...
function formatActualName(expected, actual) {
  ...
  return (
    '<span class="linter-elm-make-details-actual-name">' +
    diff
      .map(part => {
        if (part.added) {
          return (
            '<span class="linter-elm-make-actual-name-diff-changed">' +
            _.escape(part.value) +
            '</span>'
          );
        } else if (!part.removed) {
...
function formatIncorrectType(type) {
  return (
    '<div class="linter-elm-make-details-actual-type"><span class="linter-elm-make-actual-type-diff-changed">' +
    _.escape(formatType(type)) +
    '</span></span></div>'
  );
}
...
function formatCorrectName(name) {
  return (
    '<span class="linter-elm-make-details-expected-name"><span class="linter-elm-make-expected-name-diff-changed">' +
    _.escape(name) +
    '</span></span>'
  );
}
...
function formatIncorrectName(name) {
  return (
    '<span class="linter-elm-make-details-actual-name"><span class="linter-elm-make-actual-name-diff-changed">' +
    _.escape(name) +
    '</span></span>'
  );
}
...
function formatHint(hint) {
    ...
    const parts = typos
      ...
      .map(part => {
        ...
        if (partMatches && partMatches.length > 3) {
          ...
          return (
            leadingSpaces.replace(' ', '&nbsp;') +
            formatExpectedName(expected, actual) +
            ' &lt;-&gt; ' +
  ...
  if (matches && matches.length > 3) {
    return hint.replace(
      regex,
      '$1' + formatFieldsWithIssues(matches[2], formatInfo) + '$3'
    );
  }
  ...
  if (matches && matches.length > 2) {
    ...
    return hint.replace(
      regex,
      '$1' + formatFieldsWithIssues(matches[2], formatFunction)
    );
  }
  ...
  if (matches && matches.length > 5) {
    ...
    return hint.replace(
      regex,
      '$1' +
        formatFieldsWithIssues(matches[2], formatFunction1) +
        '$3' +
        formatFieldsWithIssues(matches[4], formatFunction2) +
        '$5'
    );
  }
  ...
  if (matches && matches.length > 5) {
    ...
    return hint.replace(
      regex,
      '$1' +
        formatExpectedName(numExpectedArgs, numActualArgs) +
        '$3' +
        formatActualName(numExpectedArgs, numActualArgs) +
        '$5'
    );
  }
  regex = /((?:.+)More at:\n)<(.+)>$/;
    ...
    if (nonEmptyHints.length > 0) {
      return (
        ...
        nonEmptyHints
          .map(hint => {
            return (
              '<span class="linter-elm-make-details-hint-label"></span> ' +
              '<span class="linter-elm-make-details-hint">' +
              formatHint(hint.replace('Hint: ', '')) +
              '</span>'
            );
          })
          .join('\n\n')
